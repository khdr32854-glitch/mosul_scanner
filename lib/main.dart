...
class DocumentItem {
  ...
  void replaceImage(img.Image newImage) {
    ...
    if (newImage.width > 0) {
      heightMm =
          (newImage.height / newImage.width) * widthMm;
    }
  }
}
...
class _MainScannerScreenState extends State<MainScannerScreen> {
  ...
  Future<void> _openGoogleScanner({
    ...
    try {
      final scanResult = await GoogleScanner.scan(
        fromGallery: fromGallery,
      );

      if (!mounted) return;

      // نص الخطأ الحقيقي - هذا هو المهم الآن لمعرفة السبب
      // الفعلي بدل ابتلاعه بصمت داخل debugPrint فقط.
      if (scanResult.errorMessage != null) {
        _showMessage(
          'خطأ الماسح: ${scanResult.errorMessage}',
          error: true,
        );
        return;
      }
  ...
  void _addDecodedImage(
    ...
    final encodedBytes = ImageUtils.encodeJpg(
      decodedImage,
      quality: 95,
    );
          ...
          Container(
            ...
            child: ListView(
              ...
              children: [
                ...
                _buildCamScannerFilter(
                  'Magic Pro ⭐',
                  EnhanceMode.magicPro,
                ),
                _buildCamScannerFilter(
                  'بدون ظل',
                  EnhanceMode.noShadow,
                ),
                _buildCamScannerFilter(
                  'أبيض وأسود',
                  EnhanceMode.bw,
